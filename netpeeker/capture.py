...
class WinDivertBackend(CaptureBackend):
    ...
    def _sniff_loop(self) -> None:
        try:
            ...
            print(f"[netpeeker] WinDivert sniff failed: {exc}")
            self._running.clear()
            return
        with handle:
    ...
    def _enforce_loop(self) -> None:
        ...
        # ones we allow, or they are dropped.
        try:
            handle = pydivert.WinDivert(self.SNIFF_FILTER)
        except Exception as exc:
            print(f"[netpeeker] WinDivert enforcer failed: {exc}")
            return
        with handle:
            while self._running.is_set() and self._has_rules():
                try:
                    ...
                    break
                if self._allow_packet(packet):
                    try:
                        handle.send(packet)
                    except Exception:
                        pass
                # else: dropped (blocked, or over its rate budget)
...
def make_backend(procmap: ProcessMap) -> CaptureBackend:
    ...
    if HAVE_PYDIVERT:
        try:
            ...
            print(f"[netpeeker] WinDivert unavailable ({exc}); "
